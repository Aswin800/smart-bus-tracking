  ...
  useEffect(() => {
    const checkLocation = () => {
      ...
      if (stored) {
        ...
        setBusLocation(loc);
        if (loc.speed) setStats(s => ({ ...s, speed: Math.round(loc.speed * 3.6) }));
      }
    };
      ...
      <div className="fixed inset-0 z-0 overflow-hidden">
       ...
       <div className="px-8 pb-10 relative z-50">
          <div className="glass-dark p-8 rounded-[3.5rem] border border-white/10 shadow-3xl backdrop-blur-3xl overflow-hidden relative">
             ...
             >
                EXPAND TACTICAL DATA <Navigation className="w-5 h-5 group-hover:translate-x-1 group-hover:-translate-y-1 transition-transform" />
             </motion.button>
          </div>
       </div>
